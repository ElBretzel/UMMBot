...
def extract_args(args):
    ...
    return {i: j for i, j in [i.split("=") for i in args if len(i.split("=")) > 1] if
            i in ["bg", "color", "jump", "reverse"]}


async def send_image(ctx, images, loop, executor):
    image = Image.new("RGBA", images[-1].size, "WHITE")
    # https://stackoverflow.com/questions/58664698/error-decode-byte-when-send-image-in-discord
    with BytesIO() as image_binary:
        await loop.run_in_executor(executor, lambda: image.save(fp=image_binary, format='GIF', append_images=images,
...
class GenAscii:
    ...
    async def genascii(self, ctx, *args):
        ...
        gen = int(72 // float(args.get("jump", 1)))
        message = await ctx.send(f"Progression 0/{gen} frames")
        imgs = []

        for i in range(gen, 0, -1):
            if i % 8 == 0 or i == 1:
                await message.edit(content=f"Progression: {abs(i - gen)}/{gen} frames")

            # Conversion de l'image (180px max)
            new_width = image.width if image.width <= int(i * float(args.get("jump", 1))) else int(
                i * float(args.get("jump", 1)))
