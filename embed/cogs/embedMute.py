...
class EmbedMute:
    def __init__(self, member, mod, reason, time_, sanction_id):
        self.member = member
        self.mod = mod
        self.reason = reason
        self.time_ = time_
    ...
    async def member_mute(self, color):
        embed = discord.Embed(
            title=f"⚖️ Membre mute | {await self.get_sanction_id}",
            description=f"{self.mod} a mute écrit {self.member} pendant {self.time_} pour **{self.reason}**",
            colour=color)
    ...
    async def member_vmute(self, color):
        embed = discord.Embed(
            title=f"⚖️ Membre mute | {await self.get_sanction_id}",
            description=f"{self.mod} a mute vocal {self.member} pendant {self.time_} pour **{self.reason}**",
            colour=color)
        embed = self.member_info(embed)
        return embed
    ...
    async def member_gmute(self, color):
        embed = discord.Embed(
            title=f"⚖️ Membre mute | {await self.get_sanction_id}",
            description=f"{self.mod} a mute globalement {self.member} pendant {self.time_} pour **{self.reason}**",
            colour=color)
        embed = self.member_info(embed)
        return embed
