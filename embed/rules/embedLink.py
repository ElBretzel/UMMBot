...
class Embed:
    ...
    async def link_embed(self):
        ...
        if self.type_ == "m":
            embed = await self.link_message(clock, color)

        elif self.type_ == "u":
            embed = await self.link_username(clock, color)

        return embed

    async def link_message(self, clock, color):
        tz = pytz.timezone('Europe/Paris')

        embed = discord.Embed(
            title=f"🗃️ Avertissement membre{'' if not self.sanction_id else ' | ' + str(await self.get_sanction_id)}",
            description=f"{self.member} a envoyé un message comportant un {self.detect}",
    ...
    async def link_username(self, clock, color):
        tz = pytz.timezone('Europe/Paris')

        embed = discord.Embed(
            title=f"🗃️ Avertissement membre{'' if not self.sanction_id else ' | ' + str(await self.get_sanction_id)}",
            description=f"{self.member} possède un nom d'utilisateur avec un {self.detect}",
            colour=discord.Colour.from_rgb(color[0], color[1], color[2]))
        embed.set_thumbnail(url=self.member.avatar_url)
        embed.add_field(name="🦲 Pseudo joueur:", value=f"<@{self.member.id}>", inline=True)
        embed.add_field(name="🔎 ID joueur:", value=f"{self.member.id}", inline=True)
