...
from embed.rules.embedSEmote import Embed
...
```diff
- Nous avons détecté un interval d'envoi de messages (emote) trop court. Ralentissez votre rythme pour éviter de futur avertissements.
...
rentrez la commande " -new " dans le salon commande.```""", delete_after=15)
    ...
    @property
    async def spam_emotes(self):

        if not emoji.emoji_lis(self.message.content):
            return
        ...
        cursor.execute("""
         SELECT RSpam_Emote.activate, RSpam_Emote.distance, RSpam_Emote.interval
         FROM RSpam_Emote
         INNER JOIN Guild
         ON RSpam_Emote.id = Guild.id
         WHERE Guild.id = ?
         ;
         """, (guild.id,))
        ...
        cursor.execute("""
         SELECT Text_Channel.spam_emote
         FROM Text_Channel
         INNER JOIN Channel
        ...
        emotes = [n for n, emo in enumerate(msg_list) if emoji.emoji_lis(emo.content)]
        send_time = timedelta(seconds=0)

        if len(emotes) > distance:
            for x in range(distance):
                send_time += time_list[emotes[x]] - time_list[emotes[x + 1]]
    ...
    async def rule_break(self, member, channel, log_channel_id, result, *args):
        ...
        cursor.execute("""
             SELECT Automoderation.spam_emote
             FROM Automoderation
             INNER JOIN Guild
        ...
        cursor.execute("""
             SELECT RSpam_Emote.timemute, RSpam_Emote.nbefore_mute, RSpam_Emote.distance
             FROM RSpam_Emote
             INNER JOIN Guild
             ON RSpam_Emote.id = Guild.id
             WHERE Guild.id = ?
        ...
        if automod_authorization:
            ...
            reason = 'Spam emote'
            sanction_id = await sanction_process(guild.id, 'WARN', reason, self.client.user.id, member.id, 'RSEMOTE', 0)

        if log_channel_id:
            embed = Embed(member, channel, self.message, result[0], result[1], sanction_id)
            embed = await embed.semote_embed()
            channel_send = self.client.get_channel(log_channel_id)
            await channel_send.send(embed=embed)

        if automod_authorization:
            await automod_mute_sanction(self.client, guild, member, "RSEMOTE", check_pun)
