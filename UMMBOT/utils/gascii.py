...
async def send_image(ctx, images, loop, executor):
    ...
    with BytesIO() as image_binary:
        await loop.run_in_executor(executor, lambda: image.save(fp=image_binary, format='GIF', append_images=images,
                                                                save_all=True, loop=0, optimize=True, quality=10))
        image_binary.seek(0)
...
class Gascii:
    ...
    async def gascii(self, ctx, *args):
        ...
        max_frames = min(int(64 / float(args.get("jump", 1))), image.n_frames)
        skip_frames = await loop.run_in_executor(executor, lambda: reduce_frame(image, max_frames))
        imgs = []

        message = await ctx.send(f"Progression 0/{max_frames} frames")
        ...
        for i in range(max_frames):
            ...
            if i % 4 == 0 or i == 0:
                await message.edit(content=f"Progression: {i}/{max_frames} frames")
